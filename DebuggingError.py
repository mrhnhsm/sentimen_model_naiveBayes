...
def diagnose_csv_file(file_path):
    ...
    # Coba baca file dengan berbagai encoding
            ...
            with open(file_path, 'r', encoding=enc) as f:
                ...
                for delim in delimiters:
                    if delim in first_line:
                        print(f"Delimiter terdeteksi: '{delim}'")
                
        except UnicodeDecodeError:
            print(f"Gagal membaca dengan encoding {enc}")
        except StopIteration:
            print(f"File terlalu pendek untuk dibaca dengan {enc}")
        except Exception as e:
            print(f"Error membaca file: {e}")

    return True

def read_csv_advanced(file_path):
    encodings = ['utf-8', 'latin-1', 'iso-8859-1', 'cp1252']
    delimiters = [',', ';', '\t', '|']
    ...
    for enc in encodings:
        for delim in delimiters:
            try:
                ...
                if not df.empty:
                    print(f"Berhasil membaca dengan encoding {enc} dan delimiter '{delim}'")
                    print(f"Jumlah baris: {len(df)}")
                    print(f"Kolom: {df.columns.tolist()}")
                    return df

            except Exception as e:
                print(f"Gagal dengan encoding {enc} dan delimiter '{delim}': {e}")
...
# Baca file lanjutan
try:
    df = read_csv_advanced(file_path)
except Exception as e:
    print(f"Error akhir: {e}")
